...
При старте приложения для поиска по базе документов используется ключ [key](#Key_Application) /r, анализ командной строки из консоли происходит в методе class Server::KeyApplication().
После определения кода ключа, /r keyApp = 104 в методе Run() выполняются команды поиска по заданию пользователя.
```cpp
switch(keyApp){
///...
    case (104):{
...
```
После выдачи результатов приложение переходит в режим ожидания. При изменении содержимого папки ../resources/, удалении или добавления файла, а также его редактирования и последующем сохранении
